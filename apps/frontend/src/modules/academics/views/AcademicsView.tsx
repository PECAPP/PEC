...
export function AcademicsView({ initialCourses, initialDepartments, isAdmin }: {
 ...
 const { execute: executeCreate, isPending: isCreating } = useAction(createCourseAction, {
  onSuccess: () => { toast.success('Course added successfully!'); setShowDialog(false); router.refresh(); },
  onError: ({ error }) => toast.error(error.serverError || 'Failed to add course.'),
 });
  ...
  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-10">
   ...
   <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
    <div>
     ...
     <p className="text-muted-foreground font-medium italic text-[11px] mt-1">Manage and track institutional academic courses</p>
    </div>
    <div className="flex gap-3">
     ...
     {isAdmin && (
      <Button onClick={() => { setEditingCourse(null); setCourseForm(emptyCourse); setShowDialog(true); }} className="h-11 bg-primary text-white font-bold uppercase tracking-widest text-[10px] rounded-sm px-6">
   ...
   <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
    {[
     ...
     { label: 'Departments', value: departments.length, icon: GraduationCap, color: 'success' },
     { label: 'Credits', value: courses.reduce((s: number, c: any) => s + (c.credits || 0), 0), icon: Users, color: 'warning' },
   ...
   <div className="flex flex-col md:flex-row gap-4">
    <Input placeholder="Search courses..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="flex-1 h-14 border-2 rounded-sm font-bold bg-background/50" />
   ...
   <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
    <AnimatePresence>
     {filtered.length === 0 ? (
      <div className="col-span-full p-20 text-center text-muted-foreground font-bold italic">No courses found.</div>
      ...
      >
       <div className="flex justify-between items-start mb-6">
        <div className="space-y-2">
         ...
         <h3 className="font-bold text-xl leading-tight tracking-tight">{course.name}</h3>
        </div>
        {isAdmin && (
       ...
       <div className="space-y-3">
        <div className="flex justify-between text-[10px] font-bold uppercase tracking-widest">
         <span className="text-muted-foreground/60">Department</span>
   ...
   <Dialog open={showDialog} onOpenChange={setShowDialog}>
    <DialogContent className="max-w-lg rounded-sm border-2 border-primary shadow-[20px_20px_0px_rgba(0,0,0,0.05)]">
     <DialogHeader className="space-y-3">
      <DialogTitle className="text-3xl font-bold">{editingCourse ? 'Edit Course' : 'Add New Course'}</DialogTitle>
      <DialogDescription className="text-[10px] font-bold uppercase tracking-widest bg-muted p-2 rounded-sm italic opacity-60">Academic Course Registration</DialogDescription>
     </DialogHeader>
     <div className="space-y-6 pt-4">
      ...
      <div className="space-y-2">
       ...
       <Input value={courseForm.name} onChange={e => setCourseForm({ ...courseForm, name: e.target.value })} className="h-12 border-2 rounded-sm font-bold" placeholder="Introduction to Computing" />
      </div>
      <div className="space-y-2">

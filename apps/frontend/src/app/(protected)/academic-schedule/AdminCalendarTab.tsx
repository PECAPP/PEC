'use client';
import { Button, Badge, Dialog, DialogContent, DialogTitle, DialogDescription } from "@pec/ui";
...
export default function AdminCalendarTab() {
  ...
  const columns = useMemo<ColumnDef<ParsedEvent>[]>(() => [
    {
      accessorKey: 'title',
      header: 'Title',
      cell: ({ row }) => (
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span>
          {row.original.date ? format(new Date(row.original.date), 'MMM d, yyyy') : 'N/A'}
    ...
    },
    {
      ...
      header: 'Type',
      cell: ({ row }) => <Badge variant="outline">{getEventLabel(row.original.eventType)}</Badge>
    },
    {
      accessorKey: 'category',
      header: 'Category',
      cell: ({ row }) => <span>{CATEGORY_LABELS[row.original.category] || row.original.category}</span>
    },
    {
      ...
      cell: ({ row }) => (
        <Badge
          variant="outline"
          className={
            row.original.importance === 'high'
              ? 'text-red-600'
              : row.original.importance === 'medium'
                ? 'text-yellow-600'
                : 'text-green-600'
          }
        >
    ...
    },
    {
      ...
      header: 'Location',
      cell: ({ row }) => <span className="max-w-[150px] truncate block">{row.original.location || '-'}</span>
    },
    {
      id: 'actions',
      ...
      cell: ({ row }) => (
        <div className="flex justify-end gap-2">
          <Button
            size="sm"
            variant="outline"
            onClick={() => setEditingParsedIndex(row.index)}
          >
          ...
          <Button
            size="sm"
            variant="destructive"
            onClick={() => handleDeleteParsedEvent(row.index)}
          >

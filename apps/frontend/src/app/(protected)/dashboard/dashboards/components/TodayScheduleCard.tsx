...
export function TodayScheduleCard({ scheduleDay, todayClasses, todayEvents = [], onViewFull, containerHeight }: Props) {
  ...
  return (
    <div
      className="bg-card border border-border/40 rounded-sm shadow-sm p-4 md:p-6 flex h-fit flex-col"
    ...
    >
      ...
      <div className="min-h-0 flex-1 space-y-3 overflow-y-auto pr-1">
        {todayEvents.length > 0 && (
          <div className="mb-3 rounded-sm border border-border/40 bg-primary/5 p-3">
          ...
          todayClasses.map((cls, index) => {
            ...
            return (
              <div 
                ...
                className={`rounded-sm border p-3 transition-all duration-300 ${isOngoing ? 'border-border/40 bg-primary/5 shadow-sm shadow-primary/10 ring-1 ring-primary/20' : 'border-border bg-secondary/10'}`}
              >
                <div className="mb-2 flex items-start justify-between gap-3">
                  ...
                  <Badge variant="outline" className={`shrink-0 whitespace-nowrap text-[10px] ${isOngoing ? 'bg-primary/20 border-border/40 text-primary font-bold' : 'bg-background'}`}>
                    {cls.startTime} - {cls.endTime}
                  </Badge>
                </div>
                <Link href={`/rooms`} className="flex items-center gap-1 text-[11px] font-medium  text-muted-foreground hover:text-primary transition-colors cursor-pointer group/room">

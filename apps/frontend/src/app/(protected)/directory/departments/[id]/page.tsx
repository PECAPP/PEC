    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [facultyRes, studentsRes, coursesRes, timetableRes] = await Promise.all([
          api.get(`/users?role=faculty&department=${departmentName}&limit=200`),
          api.get(`/users?role=student&department=${departmentName}&limit=500`),
          api.get(`/courses?department=${departmentName}&limit=200`),
          api.get(`/timetable?department=${departmentName}&limit=500`),
        ]);
          ...
          <div className="space-y-3 max-h-[320px] overflow-y-auto pr-2 custom-scrollbar">
            {courses.length > 0 ? (
              courses.map((course) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-sm bg-success/10 flex items-center justify-center font-bold text-success text-xs border border-success/20">
                      {course.code.slice(-3)}
                    </div>
                    <div>
              ...
              <div className="flex flex-col items-center justify-center py-10 text-muted-foreground bg-muted/10 rounded-sm">
                 <Search className="w-8 h-8 opacity-20 mb-2" />
                 <EmptyState title="No courses found" description="There are no courses currently offered by this department." />
              </div>
            )}
          </div>
        </div>
        ...
        <div className="card-elevated p-6">
          ...
          <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
            {faculty.length > 0 ? (
              ...
              <p className="text-center text-muted-foreground py-10 bg-muted/10 rounded-sm font-medium">No faculty specialized in this area</p>
            )}
          </div>
        </div>
              ...
              students.map((student) => (
                ...
                >
                  ...
                  <div className="text-right">
                    <Badge variant="secondary" className="px-2 py-0 text-[10px] uppercase font-bold bg-primary/20 text-primary border-none">Sem {student.semester || 'N/A'}</Badge>
                    <p className="text-[9px] text-muted-foreground mt-1 font-bold">BATCH 2025</p>
                  </div>
                </div>
              ))
            ) : (
              <div className="flex flex-col items-center justify-center py-12 text-muted-foreground bg-muted/10 rounded-sm">

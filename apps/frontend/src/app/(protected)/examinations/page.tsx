...
import { Course, Department, ExamSchedule } from './types';
...
export default function ExaminationsPage() {
  ...
  if (role === 'faculty' || role === 'student') {
    return <DepartmentUpcomingExams role={role} department={user.department || null} />;
  }
      ...
      <Tabs defaultValue="schedule" className="space-y-4">
        ...
        <TabsContent value="schedule" className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-6 gap-3">
            <Select
              ...
              onValueChange={(value) => setScheduleForm((p) => ({ ...p, courseId: value }))}
            >
            ...
            <Select
              ...
              onValueChange={(value) => setScheduleForm((p) => ({ ...p, examType: value }))}
            >
              <SelectTrigger>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <Select
              value={dateScope}
              onValueChange={(value) => setDateScope(value as 'all' | 'upcoming')}
            >
              <SelectTrigger>
        ...
        <TabsContent value="upload" className="space-y-3">
          ...
          <p className="text-sm text-muted-foreground">
            Upload exam schedules in bulk using `courseCode`, `examType`, `date`, `startTime`,
            `endTime`, `room`.
          </p>
        </TabsContent>
      </Tabs>
      ...
      <Dialog open={showBulkUpload} onOpenChange={setShowBulkUpload}>
        <DialogContent className="max-w-4xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Upload CSV/Excel schedules for multiple departments at once.
            </DialogDescription>
          </DialogHeader>
          <BulkUpload
            entityType="exams"
    ...
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          {role === 'faculty' ? 'Faculty' : 'Student'} view for department:{' '}
          {department || 'Not set'}
        </p>
      </div>
        ...
        <Table className="border-collapse">
          <TableHeader>
            <TableRow className="bg-muted hover:bg-muted border-b border-border/60">
              <TableHead className="w-40 font-black uppercase text-[10px] tracking-widest text-primary py-4 border-r border-border/60 text-center">
                Status & Date
              </TableHead>
              <TableHead className="font-black uppercase text-[10px] tracking-widest py-4 border-r border-border/60">
                Course Details
              </TableHead>
              <TableHead className="font-black uppercase text-[10px] tracking-widest py-4 border-r border-border/60">
                Time Slot
              ...
              <TableHead className="text-right font-black uppercase text-[10px] tracking-widest py-4">
                Type
              </TableHead>
            </TableRow>
          </TableHeader>
              ...
              if (dateKeys.length === 0) {
                return (
                  <TableRow>
                    <TableCell
                      colSpan={5}
                      className="text-center py-24 text-muted-foreground italic font-medium"
                    >
              ...
              return dateKeys.map((dateKey) => {
                ...
                return exams.map((exam, idx) => (
                  ...
                  >
                    ...
                    {idx === 0 && (
                      ...
                      >
                        <div className="flex flex-col items-center justify-center space-y-2 sticky top-6">
                          <span
                            className={`text-[10px] font-black leading-none px-2 py-1 rounded-full ${isVerySoon ? 'bg-primary text-primary-foreground shadow-lg shadow-primary/20' : 'bg-muted-foreground/20 text-muted-foreground'}`}
                          >
                            {diffDays === 0
                              ? 'TODAY'
                              : diffDays === 1
                    ...
                    <TableCell className="py-6 border-r border-border/60">
                      <div
                        className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-md border font-mono text-xs font-bold leading-none shadow-sm ${isVerySoon ? 'bg-primary/20 border-primary/30 text-primary' : 'bg-background border-border/60 text-muted-foreground'}`}
                      >
                    ...
                    <TableCell className="py-6 border-r border-border/60">
                      <div className="flex flex-col">
                        <span className="text-xs font-black text-foreground/40 uppercase tracking-widest mb-0.5">
                          Venue
                        </span>
                    ...
                    <TableCell className="py-6 text-right">
                      <Badge
                        variant={isVerySoon ? 'default' : 'outline'}
                        className="font-black uppercase text-[9px] tracking-widest px-2 shadow-sm"
                      >
                        {exam.examType}
                      </Badge>
                    </TableCell>

  ...
  if (occupied) {
    ...
    console.error(
      `Port ${PORT} is occupied by another process that is not this API. ` +
        `Free the port or set PORT to a different value in apps/server/.env.`,
    );
    process.exit(1);
  }

  const tsNodeCmd = 'ts-node --project tsconfig.json -r tsconfig-paths/register --transpile-only src/main.ts';

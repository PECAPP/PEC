...
d) Webhook handler: POST /api/webhooks/razorpay - Verify signature using Razorpay webhook secret - Update FinanceTransaction with gatewayTxnId, status - Mark FeeRecord as paid
...
c) Email templates needed: - Email verification (link with token) - Password reset (link with token) - Fee payment receipt - Attendance deficiency alert - Admin notifications
d) Hook into auth.service.ts: - After register → send verification email - After requestPasswordReset → send reset email
...
pnpm run db:reset # Wipe DB, push schema, seed data
pnpm run db:studio # Visual database browser (Prisma Studio)
pnpm run setup # Full setup: install + db:push + prisma:generate + seed
pnpm run clean:full # Remove all node_modules and build artifacts
pnpm run clean:next # Remove apps/frontend/.next cache only
pnpm run check:env # Validate all required environment variables

Database:
pnpm --filter pec-server db:push # Sync schema (dev only)
pnpm --filter pec-server db:migrate # Run prisma migrate dev
cd apps/server && npx prisma migrate deploy # Apply migrations (production)
pnpm --filter pec-server prisma:generate # Regenerate Prisma client
pnpm --filter pec-server db:seed # Seed test data
pnpm --filter pec-server db:backup # Backup database
pnpm --filter pec-server db:restore # Restore from backup

Build:
pnpm run build # Build all workspaces (frontend + backend) via Turbo
cd apps/server && npm run build # Build backend only

...
export default function MyApplications() {
  ...
  const fetchApplications = async (uid: string) => {
    try {
      ...
      const referencedJobIds = [...new Set(apps.map((appData: any) => appData.jobId).filter(Boolean))];
      ...
      if (referencedJobIds.length > 0) {
        ...
        while (keepGoing) {
          ...
          for (const job of jobs) {
            if (referencedJobIds.includes(job.id)) {
              jobsById.set(job.id, job);
            }
          }

          keepGoing = jobs.length === limit && jobsById.size < referencedJobIds.length;
          offset += limit;
        }
      }

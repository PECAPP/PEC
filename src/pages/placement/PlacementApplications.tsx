...
interface Application {
  ...
  jobTitle: string;
  companyName: string;
  status: 'pending' | 'shortlisted' | 'interviewed' | 'hired' | 'rejected' | 'withdrawn';
  appliedAt: Timestamp;
  resumeUrl: string;
  isVerified?: boolean;
  feedback?: string;
...
export default function PlacementApplications() {
  ...
  useEffect(() => {
    ...
    let q;
    if (isRecruiter) {
      q = query(collection(db, 'job_applications'), where('recruiterId', '==', user.uid), orderBy('appliedAt', 'desc'));
    } else {
      q = query(collection(db, 'job_applications'), orderBy('appliedAt', 'desc'));
    }
  ...
  const handleSaveNote = async () => {
    ...
    try {
      await updateDoc(doc(db, 'job_applications', currentAppForNote.id), {
        notes: noteContent,
        updatedAt: serverTimestamp(),
      });
  ...
  const handleStartChat = async (studentId: string, studentName: string) => {
    ...
    try {
      const participants = [user.uid, studentId].sort();
  ...
  const studentIdsKey = useMemo(() => {
    const ids = [...new Set(applications.map((app) => app.studentId).filter(Boolean))].sort();
    return ids.join('|');
  }, [applications]);
  ...
  const filteredAndSortedApps = applications
    .map(app => {
      ...
      return {
        ...
        isVerified: Boolean(profile.isVerified ?? app.isVerified),
    ...
    .sort((a, b) => {
      ...
      return (b.appliedAt?.toMillis?.() ?? 0) - (a.appliedAt?.toMillis?.() ?? 0);
    });

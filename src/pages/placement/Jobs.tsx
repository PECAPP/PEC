...
export default function Jobs() {
  ...
  const processApplication = async () => {
    ...
    if (!useExistingResume || !hasResume) {
        ...
        setUploading(true);
        try {
           const resumePathSafeName = file.name.replace(/\s+/g, '_');
           const uploadedUrl = await uploadMedia(file, user.uid);
           resumeUrlToUse = uploadedUrl || `resume://${user.uid}/${Date.now()}_${resumePathSafeName}`;
  ...
  const getCompanyImage = (company: string, title: string) => {
      const avatarName = `${company || 'Company'} ${title || ''}`.trim();
      return `https://ui-avatars.com/api/?name=${encodeURIComponent(avatarName)}&background=0D8ABC&color=fff&size=256&rounded=true&bold=true`;
  };

  const handleResumeFilePick = (nextFile?: File) => {
    if (!nextFile) return;
                ...
                <div className="flex flex-col lg:flex-row lg:items-center gap-6">
                  ...
                  <div className="flex-1 space-y-2">
                    ...
                    <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-muted-foreground font-medium">
                      ...
                      <span className="flex items-center gap-1.5"><Clock className="w-4 h-4" /> Apply by {formatDeadline(job.deadline)}</span>
                    </div>

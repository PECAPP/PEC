...
function SubmissionGradeRow({ submission, maxMarks, onGrade }: { submission: Submission; maxMarks: number; onGrade: (id: string, marks: number, feedback: string) => void }) {
  ...
  const canOpenSubmission = isSafeUrl(submission.fileUrl);
      ...
      <div className="flex justify-between items-start mb-3">
        ...
        {canOpenSubmission ? (
          ...
          <Button size="sm" variant="outline" disabled>
            <Download className="w-4 h-4 mr-2" /> Invalid Link
          </Button>
        )}
      </div>
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <Badge variant={isOverdue ? 'destructive' : isUrgent ? 'default' : 'secondary'}>
                          {daysLeft === null ? 'No deadline' : isOverdue ? 'Overdue' : `${daysLeft} days left`}
                        </Badge>
                      </div>
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            <div>
              ...
              />
              <p className="text-xs text-muted-foreground mt-1">
                Upload your file to a cloud provider and paste a public share link.
              </p>
            </div>
            <div className="flex gap-2">
              <Button

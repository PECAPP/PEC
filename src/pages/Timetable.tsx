...
export default function Timetable() {
    ...
    const loadData = async () => {
      try {
        ...
        if (user.role === 'student' && user.uid) {
          ...
          const courseIds = enrollmentsSnap.docs.map(doc => (doc.data() as any).courseId);
  ...
  const exportTimetable = () => {
    ...
    XLSX.writeFile(workbook, `timetable_export_${new Date().toISOString().split('T')[0]}.xlsx`);
    toast.success('Timetable exported successfully!');
  };

  const bulkUploadTemplate = ['day', 'timeSlot', 'courseCode', 'room'];
  const sampleBulkData = [
    { day: 'Monday', timeSlot: '09:00-10:00', courseCode: 'CS101', room: '101' }
  ];
  ...
  const handleAutoGenerate = async () => {
    ...
    try {
      ...
      const deletePromises = existingTimetable.docs.map(doc => deleteDoc(doc.ref));
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold text-foreground flex items-center gap-2">
              ...
              {isAdmin ? 'Institutional View' : isFaculty ? 'Teaching Schedule' : 'Enrolled Schedule'}
            </h3>
            {isAdmin && (
              <Badge variant="outline" className="text-[10px] uppercase font-bold">Admin Override</Badge>
            )}
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {['Computer Science & Engineering', 'Electronics & Communication Engineering', 'Electrical Engineering', 'Mechanical Engineering', 'Civil Engineering', 'Production & Industrial Engineering'].map((dept) => (
              <Button
                key={dept}
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-muted/30">
              ...
              {DAYS.map((day) => (
                <th key={day} className="border border-border p-3 text-center text-sm font-medium text-muted-foreground min-w-[150px]">
                  {day}
                </th>
              ))}
            </tr>
              ...
              <tr key={timeSlot}>
                ...
                {DAYS.map((day) => {
                  ...
                  return (
                    <td
                      key={`${day}-${timeSlot}`}
                      className={`border border-border p-2 relative group transition-colors min-h-[80px] ${isAdmin ? 'hover:bg-muted/10 cursor-pointer' : ''}`}
                    ...
                    >
                      {(() => {
                        ...
                        const filteredSlots = departmentFilter === 'all' 
                          ? roleFilteredSlots 
                          : roleFilteredSlots.filter((s: any) => s.department === departmentFilter);
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            <div>
              ...
              <Select value={slotForm.courseId} onValueChange={(value) => setSlotForm({ ...slotForm, courseId: value })}>
      ...
      <Dialog open={showBulkUpload} onOpenChange={setShowBulkUpload}>
        <DialogContent className="max-w-4xl">
          <DialogHeader>
            ...
            <DialogDescription>Upload CSV/Excel file with columns: day, timeSlot, courseCode, room</DialogDescription>
          </DialogHeader>
          <BulkUpload

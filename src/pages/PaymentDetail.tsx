...
import { db, doc, getDoc, updateDoc, serverTimestamp } from '@/lib/dataClient';
...
export default function PaymentDetail() {
  ...
  const pollTimeoutsRef = useRef<number[]>([]);
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [id, authLoading, user, isAdmin, router]);

  useEffect(() => {
    if (!showPaymentOptions || !invoice || !paymentSettings) return;
    if (paymentSettings.method !== 'upi') return;
    setTransactionId((prev) => prev || generateTransactionId());
  }, [showPaymentOptions, invoice, paymentSettings]);
  ...
  }, [invoice, paymentSettings, transactionId]);
  ...
  }, [invoice?.status, showPaymentOptions, generateQRCode]);
  ...
  const pollPaymentStatus = (invoiceId: string, maxAttempts: number, attempt: number = 0) => {
    ...
    pollTimeoutsRef.current.push(pollTimeout);
  };
    ...
    >
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="card-elevated p-6">
          ...
          {invoice.status === 'paid' && (
            <Button variant="outline" className="w-full mt-6" disabled title="Receipt download coming soon">
            ...
            {!showPaymentOptions ? (
              ...
              <div className="space-y-6">
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="bg-blue-50 dark:bg-blue-950/20 p-3 rounded-lg border border-blue-200 dark:border-blue-800">
                            <p className="text-sm text-blue-900 dark:text-blue-200">
                              ℹ️ Admin will verify your payment using the transaction ID. You'll receive confirmation via email.
                            </p>
                          </div>
                        </div>
                      </>
                ...
                <Button 
                  ...
                  disabled={confirming || processingPayment || (paymentSettings?.method === 'bank' && verificationMode && !transactionId)}
        ...
        {invoice.status === 'paid' && (
          <div className="card-elevated p-6">
            <div className="text-center py-8">
              ...
              <p className="text-sm text-muted-foreground">Transaction ID: {invoice.paymentTransactionId || 'N/A'}</p>
            </div>
          </div>
        )}
